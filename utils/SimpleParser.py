        ...
        if child.tag == 'object':
            ...
            for jj, sub_child in enumerate(child):
                ...
                if sub_child.tag == 'bndbox':  # a 'bndbox' consists of 4 integers, xmin, ymin, xmax, ymax
                    xmin = float(sub_child[0].text) / float(w)
                    ymin = float(sub_child[1].text) / float(h)
                    xmax = float(sub_child[2].text) / float(w)
                    ymax = float(sub_child[3].text) / float(h)
...
class SimpleDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        bndbox = np.array(annot[-1]).astype('float32')
...
if __name__ == '__main__':
    ...
    ds = SimpleDataset(imgdir, xmldir, transform)
